...
class ASSOWE:
    ...
    def get_status(self, point):
        ...
        return float(Weather_info[
                         (Weather_info.date_id == 2) & (Weather_info.xid == point.x) & (Weather_info.yid == point.y) & (
                         Weather_info.hour == hour)].wind)
                ...
                for x, y in zip(xs, ys):
                    ...
                    # print(self.Map[new_x][new_y].status)
                        if not self.is_valid_coord(new_x, new_y):
                            # 对于这四个点如果有代价，立马忽略。
                            self.Map[new_x][new_y].price = self.get_price(self.Map[point.x][point.y],
    ...
    def serchway(self,point):
        '''
        选取这四个点中有效点price最小的数值，如果price相同，则选取status即风速最小的值作为下一步。
        记录下这个方向的相反方向，下一次选取的时候只能选取三个方向上的点。
        '''
        #同样从起点开始也只能在四个方向上搜索。
        xs = (0, -1, 1, 0)
        ys = (-1, 0, 0, 1)
        derect = (0, 0)    #对于方向进行初始化
        while (point.x != self.endpoint.x) & (point.y != self.endpoint.y):
            for x, y in zip(xs, ys):
                if (x, y) != derect:
                    new_x, new_y = x + point.x, y + point.y
                    min_price = 999
                    if self.is_valid_coord(self.Map[new_x][new_y]):
                        if self.Map[new_x][new_y].price < min_price:
                            min_price = self.Map[new_x][new_y].price
                            derect = (x, y)
                            min_status = self.Map[new_x][new_y].status
                        elif self.Map[new_x][new_y].price == min_price:
                            if self.Map[new_x][new_y].status < min_status:
                                derect = (x, y)
                                min_status = self.Map[new_x][new_y].status
                                
            point = self.Map[new_x][new_y]
        #当四个循环完毕之后,记录下所行走的路径
            self.path.add((point.x, point.y))
